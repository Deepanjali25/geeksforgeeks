...
public class Calculator {
    ...
    public static boolean hasPrecedence(char op1, char op2) {// Returns true if 'op2' has higher or same precedence as 'op1', otherwise returns false.
        if (op2 == '(' || op2 == ')')
            return false;
        if ((op1 == '*' || op1 == '/') && (op2 == '+' || op2 == '-'))
            return false;
    ...
    public static int applyOp(char op, int b, int a) {// A utility method to apply an operator 'op' on operands 'a' and 'b'.Return the result.
        switch (op) {
            case '+':
                return a + b;
            case '-':
